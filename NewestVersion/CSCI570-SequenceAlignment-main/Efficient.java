...
public class Efficient
{
    ...
    { 
        InputExtractEfficient input = new InputExtractEfficient(args);
    ...
    public static Tuple<Integer, Alignment> findAlignment(ByteArray x, ByteArray y)                                                                                                                                             {return findAlignment(x, y, 0);}public static Tuple<Integer, Alignment> findAlignment(ByteArray x, ByteArray y, int depth)
    {
        ...
        //if x is empty but y has one character, then the alignment has a gap in x and the one character in y
            ...
            return new Tuple<>(MinimumPenaltyEfficient.pGap, new Alignment(new byte[]{GAP}, new byte[]{y.get(0)}));
        ...
        //if x has one character but y is empty, then the alignment has a gap in y and the one character in x
            ...
            return new Tuple<>(MinimumPenaltyEfficient.pGap, new Alignment(new byte[]{x.get(0)}, new byte[]{GAP}));

        //if both x and y have one character, either you match the two characters together or you have two blanks
        else if(x.length == 1 && y.length == 1)
        {
            int mismatchPenalty = MinimumPenaltyEfficient.getMismatchPenalty(x.get(0), y.get(0));

            //if the mismatch penalty has a lower cost than two blanks, then match the two characters together
            if(mismatchPenalty < 2*MinimumPenaltyEfficient.pGap)
            ...
            //if two blanks has a lower cost than the mismatch penalty, then do two blanks
                ...
                return new Tuple(MinimumPenaltyEfficient.pGap*2, new Alignment(new byte[]{x.get(0), GAP}, new byte[]{GAP, y.get(0)}));
...
class InputExtractEfficient
{
     ...
     * to be loaded
     */
    public InputExtractEfficient(String[] args)
    {
        this.args = args;
        if(args.length < 1)
 ...
 * 
 */
class MinimumPenaltyLessMemory extends MinimumPenaltyEfficient
{
...
class MinimumPenaltyEfficient
{
     ...
     */
    public MinimumPenaltyEfficient(){}
   
   
    /**
     * Construct MinimumPenalty object using String x and String y.
     */
    public MinimumPenaltyEfficient(ByteArray x, ByteArray y)
    {
        sequence1 = x;
        sequence2 = y;
...
class Alignment
{
    ...
    public int calcCost()
    {
        ...
        for(int i = 0; i < x.length(); i ++)
        {
            ...
            cost += MinimumPenaltyEfficient.getMismatchPenalty(xByte, yByte);
        }
        return cost;
    }
}
